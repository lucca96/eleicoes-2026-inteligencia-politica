...
def load_senate_dataset() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not detail_df.empty and "valorReembolsado" in detail_df.columns:
        ...
        detail_df["nome_key"] = detail_df["nome"].map(normalize_name_key)
        ceaps_totals_df = (
            detail_df.groupby("nome_key", dropna=False)
            .agg(
                valor_reembolsado_total_integrado=("valorReembolsado", "sum"),
                qtd_lancamentos_integrado=("valorReembolsado", "size"),
            )
            .reset_index()
        )
    else:
        ceaps_totals_df = pd.DataFrame(
            columns=["nome_key", "valor_reembolsado_total_integrado", "qtd_lancamentos_integrado"]
        )
    ...
    if not senate_df.empty:
        senate_df["nome_key"] = senate_df["nome"].map(normalize_name_key)
        if not ceaps_totals_df.empty:
            senate_df = senate_df.merge(ceaps_totals_df, on="nome_key", how="left")
            senate_df["valor_reembolsado_total"] = pd.to_numeric(
                senate_df.get("valor_reembolsado_total", 0), errors="coerce"
            ).fillna(0)
            senate_df["qtd_lancamentos"] = pd.to_numeric(
                senate_df.get("qtd_lancamentos", 0), errors="coerce"
            ).fillna(0)
            integrated_values = pd.to_numeric(
                senate_df["valor_reembolsado_total_integrado"], errors="coerce"
            ).fillna(0)
            integrated_counts = pd.to_numeric(
                senate_df["qtd_lancamentos_integrado"], errors="coerce"
            ).fillna(0)
            senate_df.loc[integrated_values > 0, "valor_reembolsado_total"] = integrated_values
            senate_df.loc[integrated_counts > 0, "qtd_lancamentos"] = integrated_counts
            senate_df = senate_df.drop(
                columns=["valor_reembolsado_total_integrado", "qtd_lancamentos_integrado"],
                errors="ignore",
            )
    ...
    if not detail_df.empty and "valorReembolsado" in detail_df.columns:
        ...
        detail_df["nome_key"] = detail_df["nome"].map(normalize_name_key)
        category_df = (
            detail_df.groupby(["nome_key", "categoria"], dropna=False)
            .agg(valor=("valorReembolsado", "sum"), qtd=("valorReembolsado", "size"))
            .reset_index()
        )
    else:
        category_df = pd.DataFrame(columns=["nome_key", "categoria", "valor", "qtd"])
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    p, h1, h2, h3, span, strong, label, summary, .card, .notice {{ min-width: 0; overflow-wrap: anywhere; }}
    a {{ color: var(--blue); }}
    .topbar {{
      min-height: 52px;
      display: flex;
    ...
    .filter-card summary::after {{ content: "+"; font-size: 20px; line-height: 1; }}
    .filter-card[open] summary::after {{ content: "−"; }}
    .filter-card .filters {{ padding: 0 14px 14px; }}
    ...
    .item-title {{ display: flex; justify-content: space-between; gap: 8px; align-items: start; font-weight: 820; min-width: 0; }}
    .item-title small {{ color: var(--muted); font-weight: 760; }}
    .item-grid {{ display: grid; grid-template-columns: 1fr; gap: 8px; }}
    .item-grid span {{ display: flex; justify-content: space-between; gap: 8px; color: var(--muted); font-size: 13px; min-width: 0; }}
    .item-grid b {{ color: var(--ink); }}
    .bars {{ display: grid; gap: 9px; }}
    .bar-row {{ display: grid; gap: 5px; }}
    .bar-head {{ display: flex; justify-content: space-between; gap: 8px; color: var(--muted); font-size: 12px; font-weight: 760; min-width: 0; }}
    .bar-head span {{ overflow-wrap: anywhere; }}
    ...
    @media (min-width: 760px) {{
      ...
      .filter-card summary {{ display: none; }}
      .filter-card .filters {{ padding: 14px; }}
    }}
  </style>
</head>
<body>
  ...
  <aside class="drawer" aria-label="Menu principal">
    <div class="drawer-head">
      <div><strong>Painel Político 2026</strong><span>Dados públicos legislativos</span></div>
  ...
  <header class="topbar">
    ...
    <div class="top-title"><strong>Painel Político 2026</strong><span>Câmara, Senado e ALERJ</span></div>
    <a class="author" href="https://www.linkedin.com/in/lucca-lanzellotti" target="_blank" rel="noopener">Lucca <i>in</i></a>
  </header>
  <section class="hero">
    <div class="eyebrow">Inteligência política por Lucca Lanzellotti</div>
    <h1>{title}</h1>
    <p class="subtitle">{subtitle}</p>
  </section>
  <main>{body}</main>
  <footer>
    Desenvolvido por <strong>Lucca Lanzellotti</strong>. Fontes: Câmara dos Deputados, Senado Federal, TSE, ALERJ e DOCIGP/ALERJ. Dados atualizados em {latest_data_update_label()}.
  </footer>
  <script>
    ...
    function setupFilterPanels() {{
      document.querySelectorAll('.filter-card').forEach(panel => {{
        if (window.matchMedia('(max-width: 759px)').matches) panel.removeAttribute('open');
    ...
    function renderBars(target, rows, labelField = 'label', valueField = 'value') {{
      if (!rows.length) {{
        target.innerHTML = '<p class="muted">Sem dados para o filtro atual.</p>';
        return;
      }}
    ...
    setupFilterPanels();
    window.addEventListener('resize', setupFilterPanels);
    {script}
  </script>
</body>
</html>
...
def render_landing_page(report_df: pd.DataFrame, senate_df: pd.DataFrame, rj_df: pd.DataFrame) -> str:
    ...
    party_df = (
        report_df.groupby("siglaPartido", dropna=False)
        .agg(
            gasto=("valor_liquido_total", "sum"),
            presenca=("indice_presenca_relativa", "mean"),
    ...
    state_df = (
        report_df.groupby("siglaUf", dropna=False)
        .agg(
            gasto=("valor_liquido_total", "sum"),
            presenca=("indice_presenca_relativa", "mean"),
            deputados=("idDeputado", "nunique"),
    ...
    def insight_rows(df: pd.DataFrame, label_column: str) -> str:
        rows = []
        for row in df.to_dict(orient="records"):
            rows.append(
                '<div class="insight-row">'
                f"<div><strong>{html.escape(str(row[label_column]))}</strong>"
                f"<span>{int(row['deputados'])} deputados · presença média {float(row['presenca'] or 0):.1f}%</span></div>"
                f"<b>{compact_money_label(float(row['gasto'] or 0))}</b>"
                "</div>"
            )
        return "\n".join(rows)
    body = f"""
    <section class="grid landing-grid">
      <a class="card landing-card accent-blue" href="deputados-federais.html">
        ...
        <div class="metric-sub">Gasto parlamentar, presença, ausências e votações em PECs.</div>
      </a>
    ...
    <section class="card accent-blue">
      ...
      <p>Quem usa mais recurso público? Quem aparece menos nos registros disponíveis de trabalho? Quais categorias concentram os gastos quando um parlamentar específico é filtrado?</p>
      <p class="muted">O painel aponta padrões para fiscalização cidadã. Maior gasto ou menor presença não comprova irregularidade isoladamente.</p>
    </section>
    <section class="grid landing-grid">
      <div class="card"><div class="metric-label">Federais abaixo da mediana de presença relativa</div><div class="metric-value">{low_presence_count}</div></div>
      ...
      <div class="card"><div class="metric-label">Senadores com cadastro no período</div><div class="metric-value">{len(senate_df)}</div></div>
    </section>
    <section class="grid two-col">
      <div class="card">
        <h2>Gasto federal por partido</h2>
        <p class="source-line">Visão agregada para comparar blocos políticos antes de entrar em deputados individuais.</p>
        <div class="insight-list">{insight_rows(party_df, "siglaPartido")}</div>
      </div>
      <div class="card">
        <h2>Gasto federal por estado</h2>
        <p class="source-line">Recorte por UF com gasto parlamentar total e presença média relativa.</p>
    ...
    return site_shell(
        ...
        "Gasto parlamentar e sinais de comprometimento com o trabalho em dados públicos legislativos.",
        "home",
        body,
    )
...
def render_federal_page(report_df: pd.DataFrame, category_df: pd.DataFrame) -> str:
    ...
    <details class="card accent-blue filter-card" open>
      <summary>Filtrar deputados federais</summary>
      <div class="filters">
        ...
        <label>Ordenar<select id="sortFilter">
          ...
          <option value="valor_liquido_total">Gasto parlamentar</option>
          <option value="indice_presenca_relativa">Presença relativa</option>
          <option value="pct_ausencia_nao_justificada">Ausência não justificada</option>
        </select></label>
      </div>
    </details>
    <section class="grid two-col">
      <div class="card">
        <h2>Deputados por gasto e presença</h2>
    ...
    function renderPage() {{
      ...
      document.getElementById('kpis').innerHTML = `
        ...
        <div class="card"><div class="metric-label">Presença relativa média</div><div class="metric-value">${{pct.format(avg(rows, 'indice_presenca_relativa'))}}%</div></div>
        <div class="card"><div class="metric-label">Ausência não justificada média</div><div class="metric-value">${{pct.format(avg(rows, 'pct_ausencia_nao_justificada'))}}%</div></div>
      `;
      document.getElementById('memberList').innerHTML = rows.slice(0, 25).map(row => `
        <article class="item">
          ...
          <div class="item-grid">
            ...
            <span>Presença relativa <b>${{pct.format(Number(row.indice_presenca_relativa || 0))}}%</b></span>
            <span>Ausência não just. <b>${{pct.format(Number(row.pct_ausencia_nao_justificada || 0))}}%</b></span>
          </div>
        </article>
      `).join('');
    ...
    return site_shell(
        "Deputados federais",
        "Gasto parlamentar, presença, ausências e votações em PECs dos deputados federais.",
        "federal",
        body,
        script,
    )
...
def render_senate_page(senate_df: pd.DataFrame, category_df: pd.DataFrame) -> str:
    body = """
    <section class="grid kpis" id="kpis"></section>
    <details class="card accent-green filter-card" open>
      <summary>Filtrar senadores</summary>
      <div class="filters">
        ...
        <label>Ordenar<select id="sortFilter">
          <option value="valor_reembolsado_total">Gasto parlamentar</option>
          <option value="qtd_votacoes">Votações nominais</option>
          <option value="qtd_votos">Votos registrados</option>
        </select></label>
      </div>
    </details>
    <section class="grid two-col">
      <div class="card"><h2>Senadores por gasto e votações</h2><div class="list" id="memberList"></div></div>
      <div class="card"><h2>Categorias do gasto parlamentar</h2><p class="source-line">CEAPS consolidada a partir do arquivo granular de despesas; as categorias mudam conforme o senador filtrado.</p><div class="bars" id="categoryBars"></div></div>
    </section>
    ...
    function filteredCategories(rows) {{
      const names = new Set(rows.map(row => String(row.nome_key || '').toUpperCase()));
      const grouped = new Map();
      CATEGORIES.filter(row => names.has(String(row.nome_key || '').toUpperCase())).forEach(row => {{
        const current = grouped.get(row.categoria) || {{ label: row.categoria, value: 0 }};
        current.value += Number(row.valor || 0);
        grouped.set(row.categoria, current);
      }});
    ...
    function renderPage() {{
      ...
      document.getElementById('kpis').innerHTML = `
        ...
        <div class="card"><div class="metric-label">Votações nominais</div><div class="metric-value">${{number.format(sum(rows, 'qtd_votacoes'))}}</div></div>
      ...
      document.getElementById('memberList').innerHTML = rows.slice(0, 25).map(row => `
        <article class="item">
          ...
          <div class="item-grid">
            ...
            <span>Lançamentos <b>${{number.format(Number(row.qtd_lancamentos || 0))}}</b></span>
            <span>Votações <b>${{number.format(Number(row.qtd_votacoes || 0))}}</b></span>
      ...
      renderBars(document.getElementById('categoryBars'), filteredCategories(rows));
      document.getElementById('scriptText').value = first.nome
        ? `Senador ${{first.nome}}, do ${{first.siglaPartido}}-${{first.siglaUf}}: gasto parlamentar CEAPS de ${{money(first.valor_reembolsado_total)}} e ${{number.format(Number(first.qtd_votacoes || 0))}} votações nominais registradas. Fonte: dados públicos organizados por Lucca Lanzellotti.`
    ...
    renderPage();
    """
    return site_shell("Senadores", "Gasto parlamentar CEAPS e votos nominais dos senadores em exercício.", "senado", body, script)


def render_state_page(rj_df: pd.DataFrame, category_df: pd.DataFrame) -> str:
    body = """
    <section class="grid kpis" id="kpis"></section>
    <details class="card accent-yellow filter-card" open>
      <summary>Filtrar deputados estaduais</summary>
      <div class="filters">
        ...
        <label>Ordenar<select id="sortFilter">
          <option value="valorGastoParlamentar">Gasto parlamentar</option>
          <option value="qtdLancamentos">Lançamentos</option>
          <option value="qtdFornecedores">Fornecedores</option>
          <option value="usoLimitePct">Uso do limite</option>
        </select></label>
      </div>
    </details>
    <section class="notice">A ALERJ/DOCIGP já permite analisar gasto parlamentar estadual por deputado e categoria. Presença e votações estaduais ainda dependem de extração estruturada; por isso não aparecem como indicador de comprometimento nesta página.</section>
    <section class="grid two-col">
    ...
    function renderPage() {{
      ...
      document.getElementById('kpis').innerHTML = `
        ...
        <div class="card"><div class="metric-label">Lançamentos</div><div class="metric-value">${{number.format(sum(rows, 'qtdLancamentos'))}}</div></div>
      ...
      document.getElementById('memberList').innerHTML = rows.slice(0, 25).map(row => `
        <article class="item">
          ...
          <div class="item-grid">
            ...
            <span>Lançamentos <b>${{number.format(Number(row.qtdLancamentos || 0))}}</b></span>
      ...
      document.getElementById('scriptText').value = first.nomeParlamentar
        ? `Deputado estadual ${{first.nomeParlamentar}}, do ${{first.siglaPartido || 'partido não identificado'}}: gasto parlamentar DOCIGP de ${{money(first.valorGastoParlamentar)}} em ${{number.format(Number(first.qtdLancamentos || 0))}} lançamentos. Presença e votações estaduais ainda precisam de fonte estruturada para comparação justa.`
...
def render_methodology_page() -> str:
    ...
    <section class="card accent-blue">
      <h2>Metodologia</h2>
      <p>O painel separa casas legislativas e fontes para evitar comparações falsas. Gasto parlamentar, remuneração, presença e votações só são exibidos quando há fonte estruturada suficiente.</p>
      <p>Maior gasto parlamentar não significa irregularidade. Menor presença ou menor volume de votações também precisa de contexto, agenda, licenças, justificativas e fonte oficial.</p>
    </section>
    <section class="grid landing-grid">
      <div class="card"><h3>Câmara dos Deputados</h3><p>Dados Abertos da Câmara, cota parlamentar, remuneração estimada, presença e votações em PECs.</p></div>
      <div class="card"><h3>Senado Federal</h3><p>CEAPS e votações nominais dos senadores em exercício.</p></div>
      <div class="card"><h3>ALERJ/DOCIGP</h3><p>Descentralização Orçamentária de Custeio Individualizado para Gabinete Parlamentar. Fontes documentais da ALERJ ficam apenas como referência de auditoria, não como página analítica.</p></div>
    </section>
    <section class="card">
    ...
    """
    return site_shell("Metodologia e fontes", "Fontes, limites e critério de leitura dos indicadores.", "metodologia", body)
...
<body>
  <p>Redirecionando para <a href="index.html">Painel de Inteligência Política 2026</a>.</p>
</body>
</html>
"""
